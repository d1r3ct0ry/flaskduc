...
# Função helper para adicionar CORS manualmente
...
def calcular_frete():
    ...
    payload = {
        ...
        "package": {"height": pacote.get("height",2), "width": pacote.get("width",11), "length": pacote.get("length",16), "weight": pacote.get("weight",0.3)}
    }

    try:
        response = session.post(SUPERFRETE_URL, headers={"Content-Type":"application/json"}, data=json.dumps(payload), timeout=10)
